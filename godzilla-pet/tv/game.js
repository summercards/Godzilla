...
if(restore&&restore.district===data.district){p.x=clamp(Number(restore.x)||420,420,LENGTH+Math.max(0,worldChunk-2)*CHUNK_SPAN+CHUNK_SPAN);let states=new Map((Array.isArray(restore.buildings)?restore.buildings:[]).map(b=>[b.id,b]));for(let b of buildings){let s=states.get(b.id);if(s){let oldMax=Number(s.max)>0?Number(s.max):(b.layer===1?230:b.layer===0?120:85)*(1+Math.max(0,data.district-1)*.12);b.hp=b.max*clamp((Number(s.hp)||0)/oldMax,0,1);b.dead=s.dead===true||b.hp<=0;b.collapse=b.dead?3:0;}}let es=new Map((Array.isArray(restore.enemies)?restore.enemies:[]).map(e=>[e.id,e]));for(let e of enemies){let s=es.get(e.id);if(s){e.hp=clamp(Number(s.hp)||0,0,e.max);if(e.type==='sentinel'){e.introDone=s.introDone!==false;e.introStarted=s.introStarted===true;e.introTime=clamp(Number(s.introTime)||0,0,window.SentinelBoss.INTRO.duration);}if(s.state!=='alive'||e.hp<=0)e.state='gone';}}}
if(worldChunk>1){buildings=buildings.filter(b=>b.x>p.x-2200);enemies=enemies.filter(e=>e.x>p.x-2200||alive(e)&&Math.abs(e.x-p.x)<1350);}camera=cameraTarget();$('location').textContent=cityTitle();refreshTicker();commitTicker();tickerOffset=0;spawnTimer=7;if(restore?.nextDistrict>data.district){enemies.filter(e=>e.gate).forEach(e=>{e.hp=0;e.state='gone';});announceMapGate();}}
...
function defeat(e,source='beam'){if(!alive(e))return;e.hp=0;e.age=0;e.hit=0;data.kills++;grant(TYPES[e.type].reward,P.xpPerEnemy(data.district),e.x,e.y-50);if(e.type==='sentinel'){e.state='falling';e.age=0;shake=12;broadcast('巨型守卫倒下，关卡出口已解除封锁','银曜巨人核心熄灭 · 防线突破');return;}let flying=/heli|gunship/.test(e.type);if(source==='claw'||source==='tail'){e.state='flying';e.vx=rand(230,370)*(source==='tail'?-1:1);e.vy=-rand(250,390);e.spin=(source==='tail'?-1:1)*rand(4,8);burst(e.x,e.y,14,undefined,130);broadcast(flying?'直升机被巨兽击飞，正在失控翻滚':'装甲车辆被拍向半空，残骸高速翻滚','现场画面：目标将在落地时发生二次爆炸');}else if(flying){e.state='crashing';e.vx=rand(-100,160);e.vy=rand(0,50);e.spin=rand(1.8,3.5);explosion(e.x,e.y,.65);broadcast('武装直升机失控，拖着浓烟坠向街区','地面机位追踪中 · 旋翼损毁，机身持续旋转');}else{e.state='exploding';e.rotation=rand(-.12,.12);e.age=0;explosion(e.x,e.y,1.1);fires.push({x:e.x,y:G-12,size:33,age:0,jitter:rnd()*.2-.1});}}
...
function updateAI(dt){if(enemies.some(e=>alive(e)&&e.type==='sentinel'&&e.introStarted&&!e.introDone)){p.moving=false;return;}if(p.stagger>0){p.stagger=Math.max(0,p.stagger-dt);p.moving=false;return;}for(let k in p.cooldowns)p.cooldowns[k]=Math.max(0,p.cooldowns[k]-dt);p.stagger=Math.max(0,p.stagger-dt);let a=p.action;a.t+=dt;
...
ensureWorldAhead();if(!mapGate&&mapProgress()>=1&&!enemies.some(e=>e.gate&&alive(e))){announceMapGate();banner('已突破当前地图','继续推进中 · 顶部按钮可进入下一张地图');save();}rigState=KaijuRig.pose(p,time);BS=bodyScale();SK=scaleRig(rigState,BS,p.x,G);SM=SK.muzzle;}
...
function updateBeam(dt){beam=null;arcs=[];if(enemies.some(e=>alive(e)&&e.type==='sentinel'&&e.introStarted&&!e.introDone))return;let a=p.action;if(a.name!=='beam'||a.t<.95||a.t>3.55)return;let {x,y}=(SM||rigState.muzzle),dx=Math.cos(p.angle),dy=Math.sin(p.angle),hits=[];for(let b of buildings){if(b.dead)continue;let distance=rayBox(x,y,dx,dy,b);if(distance!==null&&distance>0&&distance<1100)hits.push({distance,target:b});}for(let e of enemies){if(!alive(e))continue;let ex=e.x-x,ey=e.y-y,d=ex*dx+ey*dy,off=Math.abs(ex*dy-ey*dx);if(d>0&&d<1100&&off<(e.type==='sentinel'?185:e.type==='mech'?90:/heli|gunship/.test(e.type)?34:42))hits.push({distance:d,target:e});}hits.sort((a,b)=>a.distance-b.distance);let chosen=hits.slice(0,economy.has('pierce')?3:1),reach=chosen.length?chosen.at(-1).distance:1000;beam={x,y,ex:x+dx*reach,ey:y+dy*reach,super:a.super};let fire=data.subElements&&data.subElements.includes('pyro');beam.fire=fire;
...
$('nextMap').onclick=()=>{if(panelMode){sendPanelCommand({id:'nextMap'});return;}if(requestNextMap()){notice('已进入下一张地图：'+cityTitle());hud();}else notice('当前地图尚未突破');};
...
function drawAssignHologram(c,rig,t){
  ...
  const fit=Math.min(280/((bounds.x1-bounds.x0)*Growth.CEIL),192/((bounds.y1-bounds.y0)*Growth.CEIL));
...
for(let f of fires){f.age+=dt;const smokeRate=data.level<15?5.5:11;if(Math.random()<dt*smokeRate){const smokeScale=clamp(bodyScale()*(1+(f.jitter||0)),.25,1.25);smoke(f.x+rand(-f.size/2,f.size/2),f.y-20,20*smokeScale);}}fires=fires.filter(f=>f.age<24&&Math.abs(f.x-p.x)<1900).slice(-60);for(let w of wrecks)w.age+=dt;wrecks=wrecks.filter(w=>w.age<50&&Math.abs(w.x-p.x)<1900).slice(-40);
